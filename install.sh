...
print(int(float(r[0]['value'][1]))) if r else print(0)" 2>/dev/null || echo 0
}

PEAK_USERS=$(prom_query "ceil(max_over_time(sum(mtg_client_connections)[24h:1m]) / 4)")
AVG_USERS=$(prom_query "ceil(avg_over_time(sum(mtg_client_connections)[24h:1m]) / 4)")
TRAFFIC_24H=$(prom_query "sum(increase(mtg_telegram_traffic{direction="from_client"}[24h]))")
TRAFFIC_30D=$(prom_query "sum(increase(mtg_telegram_traffic{direction="from_client"}[30d]))")
...
if b>1099511627776: print(f'{b/1099511627776:.2f} TB')
elif b>1073741824: print(f'{b/1073741824:.1f} GB')
elif b>1048576: print(f'{b/1048576:.1f} MB')
elif b>1024: print(f'{b/1024:.1f} KB')
else: print(f'{b} B')" 2>/dev/null || echo "${1}B"; }
...
$(echo -e "$INST_LINES")
📦 <b>Трафик</b>
За 24ч: <b>$(fmt_total $TRAFFIC_24H)</b>
За 30д: <b>$(fmt_total $TRAFFIC_30D)</b>
